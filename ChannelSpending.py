# T10Metrics.py
# Core metrics (candidate & race) with NaN-safe channel refinement:
...
# ----- Optional ML (falls back if sklearn isn't available) -----
SKLEARN_OK = True
try:
...
ENABLE_ML = True          # auto-skips if sklearn missing / data insufficient
...
# Ensure/normalize key text cols (avoid float NaNs -> 'lower' crash)
...
# ===================== Channel Refinement =====================
# 0) Start: if existing channel present use it, else compute base
...
# 2) Vendor backfill on remaining unclear
...
# 3) Optional ML on remaining unclear
still_unclear = raw["channel_refined"].eq("Other/Unclear")
...
# Small refinement summary
base_other = int((raw["channel_base"] == "Other/Unclear").sum())
final_other = int((raw["channel_final"] == "Other/Unclear").sum())
pd.DataFrame(
    [("base_other_rows", base_other),
     ("final_other_rows", final_other),
     ("delta_other_rows", base_other - final_other)],
...
for keys, df_sub in cand_group:
    ...
    rec.update(compute_channel_mix(df_sub))  # uses channel_final
